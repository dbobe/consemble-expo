...
export default function NewQuestsPage() {
  ...
  const hasSetInitialQuest = useRef(false);
  ...
  // Select actual quest
  useEffect(() => {
    if (availableQuests.length > 0 && !currentCard && !isAnimating && !hasSetInitialQuest.current) {
      const persistedQuest = me?.root?.currentQuest;
      let questToShow: Quest | null = null;

      if (persistedQuest && availableQuests.some((q) => q.title === persistedQuest.title)) {
        questToShow = persistedQuest;
      } else {
        questToShow = availableQuests[Math.floor(Math.random() * availableQuests.length)];
      }
                  ...
                  <View className="p-6">
                    ...
                    <View className="flex-row justify-center items-center gap-6 mb-8">
                      ...
                      <View className="flex-row items-center gap-2">
                        ...
                        >
                          {currentCard?.quest?.difficulty.charAt(0).toUpperCase() +
                            currentCard?.quest?.difficulty.slice(1)}
                        </Text>
                      </View>
                      <QuestVibeScore vibeScore={currentCard?.quest?.vibeScore} />
                    </View>
                    {/* Action Buttons */}
